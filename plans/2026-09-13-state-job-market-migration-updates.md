...
### Task 8: Rewrite `src/pages/MarketUpdates.jsx` [x]
...
- **Implementation (green):** Rewrite `src/pages/MarketUpdates.jsx`:
  ...
  - On mount (signed-in): `getProfile(user.id)` → normalise to `{ targetOccupation, state, studyStage, graduationYear, courseLengthYears }` (Phase 1 correction: the original plan normalised to just `{ targetOccupation, state }`, but `pickCurrentPeriod` — called from `handleAddToPlan` below — needs the study-timeline fields too, via `getExpectedPeriodLabels`; without them it crashes on an empty period list); `getPlanWithActivities(user.id)` → `plan` (nullable); `getSavedMarketUpdates(user.id)` → map to `{ [source_id]: status }`.
  - Compute `matched = marketSources.filter((item) => matchesProfile(item, normalisedProfile))`.
  - `Promise.all(matched.map(async (item) => { const text = await summariseMarketUpdate(item); const parsed = JSON.parse(text); return isValidClassifiedItem(parsed) ? { ...item, ...parsed } : null }))`, filter out `null`s and any whose `source_id` status is `'dismissed'`. (`summariseMarketUpdate` resolves the raw JSON string directly — no `{ text }` destructuring — per the corrected §3 note above.) **Post-acceptance manual-verification fix:** against the live app, Gemini sometimes wraps its JSON in a ` ```json ... ``` ` code fence despite the prompt saying not to — `JSON.parse` on the raw text then threw, and because it was inside `Promise.all`, one bad item hung the *entire* feed forever on "Loading updates…". Each item's classify step is now wrapped in try/catch (stripping a leading/trailing code fence before parsing; a parse failure excludes just that item, matching the "excluded rather than shown" handling of an out-of-enum item).
  ...
  - Empty states: if the post-classification `classified.length === 0` → "Nothing has been curated for your profile yet." else if the filtered list is empty → "No updates match your current filters." (Phase 1 correction: gated on `classified.length`, not raw `matched.length` — the "excludes an item whose classified topic is outside the fixed enum" test expects the "nothing curated" copy when the one matched source item is excluded for being out-of-enum, not the "no filter matches" copy.)
  - Disclaimer footer: "This is general career information, not financial, investment, legal or migration advice."
  - Handlers: `handleSave`/`handleDismiss` fire `setMarketUpdateStatus(user.id, item.id, status)` without awaiting it (Phase 1 correction: the original plan awaited it before updating local state, but the "dismisses a card ... and removes it from the feed" test asserts the card is gone from the DOM immediately after the synchronous `fireEvent.click`, with no `waitFor`/`await` — an awaited persistence call delays the UI update past that assertion) and update local `savedMap`/remove from list synchronously; `handleAddToPlan` computes `pickCurrentPeriod(normalisedProfile)` from `roadmap.js`, then calls `addPlanActivityFromUpdate(plan.id, user.id, { title: item.headline, category: 'Commercial and industry awareness', priority: 'Medium', explanation: item.whyItMatters, ...period })`; `handleFrequencyChange` calls `setUpdateFrequency(user.id, value)`.
- **Refactor:** None expected.
